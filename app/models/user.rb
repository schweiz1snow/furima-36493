class User < ApplicationRecord
  ...
  validates :first_name_kana, presence: true, format: {with: /\A[ァ-ヶー－]+\z/, message: "お名前（カナ）は全角カタカナ"}
  validates :last_name_kana, presence: true, format: {with: /\A[ァ-ヶー－]+\z/, message: "お名前（カナ）は全角カタカナ"}
